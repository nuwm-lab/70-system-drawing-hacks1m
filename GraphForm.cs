    ...
    public class GraphForm : Form
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                ...
                // Захист від нульових або від'ємних розмірів
                if (client.Width <= 0 || client.Height <= 0)
                    return;

                int plotWidth = Math.Max(1, client.Width - margin.Left - margin.Right);
                int plotHeight = Math.Max(1, client.Height - margin.Top - margin.Bottom);
                Rectangle plotRect = new Rectangle(margin.Left, margin.Top, plotWidth, plotHeight);

                // Перевірка на мінімальні розміри
                if (plotRect.Width <= 50 || plotRect.Height <= 50)
                {
                    ...
                    using (var brush = new SolidBrush(Color.Gray))
                    {
                        ...
                        float x = Math.Max(0, (client.Width - sz.Width) / 2);
                        float y = Math.Max(0, (client.Height - sz.Height) / 2);
            ...
            {
                // Показуємо помилку для діагностики
                using (var font = new Font("Arial", 8))
                using (var brush = new SolidBrush(Color.Red))
                {
                    g.DrawString("Помилка: " + ex.Message, font, brush, 10, 10);
                }
            }
        }
    }
